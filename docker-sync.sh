#!/bin/bash
#
# A script forked from docker-osx-dev, @see https://github.com/brikis98/docker-osx-dev
# to work alongside the dinghy project.
# The main differences with the original script are :
# - content created in docker containers are no more deleted after a sync,
# - configuration of volume synced is now done with special comments in compose-file
#
# Example of configuration in your docker-compose.yml file
#
##rsync:
##- ./path/to/sync:full_container_name:/path/to/sync/in/container
set -e
...
readonly DEFAULT_PATHS_TO_SYNC="./src"
readonly DEFAULT_EXCLUDES=".git"
readonly DEFAULT_IGNORE_FILE=".dockerignore"
readonly RSYNC_FLAGS="--archive --log-format 'Syncing %n: %i' --omit-dir-times --inplace --whole-file -l"
...
#
# Returns true if the docker-machine VM is running
#
...
#
# Usage: init_docker_host
#
# Launches dinghy
# Init docker machine
#
function init_docker_host {
  if [[ -z "$DOCKER_MACHINE_NAME" ]] || dinghy status | grep -q "stopped"; then
     dinghy start
  fi
...
#
# Usage: find_path_to_read_index PATH
#
# Finds the parent folder of PATH from the PATHS_TO_READ global variable. When
...
#
function find_path_to_read_index {
  local readonly path="$1"
  local readonly normalized_path=$(greadlink -m $(eval echo '$path'))
  local readonly paths_to_read=($PATHS_TO_READ)

  local index=""
  for index in ${!paths_to_read[*]}; do
    path_to_sync=${paths_to_read[$index]}
...
function do_delete {
  ...
  local readonly path="$1"
  local readonly index=$(find_path_to_read_index "$path")
  local readonly path_to_read="${PATHS_TO_READ[$index]}/"

  if [[ -z "$path_to_read" ]]; then
    log_error "Internal error: can't sync '$path' because it doesn't seem to be part of any paths configured for syncing: $PATHS_TO_READ"
  else
    local readonly path_to_write="${PATHS_TO_WRITE[$index]}/"

    local readony relative_path=${path#$path_to_read}
    local readonly ssh="ssh -i $DOCKER_HOST_SSH_KEY -o IdentitiesOnly=yes -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null $DOCKER_HOST_SSH_URL "
    local readonly delete=" sudo rm -rfv ${path_to_write}${relative_path}"
    local readonly cmd="${ssh} ${delete} "
    log_debug "$cmd"
    eval "$cmd" 2>&1 | grep -v "^Warning: Permanently added" | log_info
  fi
}
...
function do_rsync {
  ...
  if [[ -z "$path_to_read" ]]; then
    ...
    local readonly rsync_cmd="rsync --rsync-path='sudo rsync' $RSYNC_FLAGS $include_flags $exclude_flags $rsh_flag $path_to_read $DOCKER_HOST_SSH_URL:$path_to_write 2>&1 | grep -v \"^Warning: Permanently added\""
    log_debug "$rsync_cmd"
...
function tar_sync {
  local readonly paths_to_read=($PATHS_TO_READ)
  local readonly paths_to_write=($PATHS_TO_WRITE)
  ...
  for index in ${!paths_to_read[*]}; do
    path=${paths_to_read[$index]}
    parent_dir=$(dirname "$path")
    base_name=$(basename "$path")
    parent_dir_path_to_write=$(dirname "${paths_to_write[$index]}")
    destination_dir=
    log_info "Initial sync using tar for $parent_dir/$base_name in ${paths_to_write[$index]}"
    tar -cC "$parent_dir" $include_flags $exclude_flags "$base_name" | $DOCKER_HOST_SSH_COMMAND "sudo tar -xC '${parent_dir_path_to_write}'"
  done
}
...
function initial_sync {
  local readonly paths_to_read=($PATHS_TO_READ)
  local readonly paths_to_write=($PATHS_TO_WRITE)
  log_info "Performing initial sync of paths:"

  for index in ${!paths_to_read[*]}; do
    echo "${paths_to_read[$index]} in ${paths_to_write[$index]}"
  done

  tar_sync

  log_debug "Starting sync paths"
  do_sync
...
# Usage: watch
#
# Watches the paths in the global variable PATHS_TO_READ for changes and rsyncs
# any files that changed.
#
function watch {
  log_info "Watching: $PATHS_TO_READ"

  local readonly fswatch_cmd="fswatch -Ie \".*\/\.git\/.*\" -Ie \".*\/\.idea\/.*\" -0x $PATHS_TO_READ"
  ...
  eval "$fswatch_cmd" | while read -d "" notification
  do
    file=$(echo $notification | sed -ne "s/^\([^ ]*\).*$/\1/p")
    event=$(echo $notification | sed -ne "s/^[^ ]* \(.*\) [^ ]*$/\1/p")
    filetype=$(echo $notification | sed -ne "s/^.* \([^ ]*\)$/\1/p")
    [[ "$event" == *Removed* ]] && isDeleted=true
    [[ "$event" == *Created* ]] && isCreated=true
    log_debug "$notification"
    log_debug "Event: $event > deleted: $isDeleted > created: $isCreated"
    if $isDeleted; then
        do_delete "$file"
    else
        do_rsync "$file"
...
function instructions {
  echo -e
  echo -e "Usage: docker-sync [COMMAND] [OPTIONS]"
  echo -e
  echo -e "Commands:"
  echo -e "  $SYNC_COMMAND\t\tStart file syncing. This is the default if no COMMAND is specified."
  echo -e "  $WATCH_ONLY_COMMAND\tWatch the file system for changes, without an initial sync."
  echo -e "  $SYNC_ONLY_COMMAND\tSync the file system and exit, without watching afterwords."
  echo -e "  $INSTALL_COMMAND\tInstall docker-sync and all of its dependencies."
  echo -e
  echo -e "Options:"
  ...
  echo -e "  -c, --compose-file COMPOSE_FILE\tRead configuration in this docker-compose file. Default: $DEFAULT_COMPOSE_FILE"
  ...
  echo -e "Overview:"
  echo -e
  echo -e "docker-sync is a script, forked from docker-osx-dev, and modified in order to work alongside the dinghy prohect."
  echo -e "You can use to sync folders from the host (your Mac) to any a specified volume inside a docker container, using rsync."
  echo -e "It's a complementary tool of the default NFS volumes mount of the dinghy project, and may be used if the performance of the NFS mount is not sufficient for your use."
  echo -e "Typical use case can be to share a large code base inside a web-server root folder."
  echo -e
  echo -e "Example workflow:"
  echo -e "  > configure your docker-compose file"
  echo -e "  > start your containers first: docker-compose up -d"
  echo -e "  > launch the synchronisation script: docker-sync -c docker-compose.yml"
  echo -e
  echo -e "  After you run the commands above, each configured volume will be synced with the host. Note that it is a one way sync from your host to the container."
  echo -e "  Files created in the container will not be synced back to the host. If you need two-way sync, you can use the docker image mickaelperrin/lsyncd to get changes back."
  echo -e
  echo -e " Example docker-file with two way sync:"
}
      ...
      if $in_volumes_block; then
        if [[ "${line:0:2}" = "#-" ]]; then
          if [[ $action == "read" ]]; then
            ...
            local readonly container=$(echo $line | sed -ne "s/#- [^:]*:\([^:]*\):.*$/\1/p" | sed -e 's/^["'\'']//'  -e 's/["'\'']$//')
            local readonly volume=$(echo $line | sed -ne "s/#- [^:]*:[^:]*:\([^:]*\)$/\1/p" | sed -e 's/^["'\'']//'  -e 's/["'\'']$//')
            local readonly path=$(eval "docker inspect --format '{{ range .Mounts }}{{ if eq .Destination \"$volume\" }}{{ .Source }}{{ end }}{{ end }}' $container")
          fi
          if [ ! -z "$path" ]; then
            paths+=("$path")
          fi
...
function configure_paths_to_sync {
  ...
  local readonly paths_to_read_from_compose_file=($(load_paths_from_docker_compose "$compose_file" "read"))
  local readonly paths_to_write_from_compose_file=($(load_paths_from_docker_compose "$compose_file" "write"))
...
#  fi
  ...
  if [[ "${#paths_to_read_from_compose_file}" -gt 0 ]]; then
    log_info "Using read paths from Docker Compose file at $compose_file: ${paths_to_read_from_compose_file[@]}"
    paths_to_read+=("${paths_to_read_from_compose_file[@]}")
  fi

  if [[ "${#paths_to_write_from_compose_file}" -gt 0 ]]; then
    log_info "Using write paths from Docker Compose file at $compose_file: ${paths_to_write_from_compose_file[@]}"
    paths_to_write+=("${paths_to_write_from_compose_file[@]}")
  fi
...
#  fi
  ...
  local normalized_paths_to_read=()
  local path=""
  for path in "${paths_to_read[@]}"; do
    local normalized_path=$(greadlink -m $(eval echo "$path"))
    normalized_paths_to_read+=("$normalized_path")
  done
  for path in "${paths_to_write[@]}"; do
    local normalized_path=$(greadlink -m $(eval echo "$path"))
    normalized_paths_to_write+=("$normalized_path")
  done

  PATHS_TO_READ="${normalized_paths_to_read[@]}"
  PATHS_TO_WRITE="${normalized_paths_to_write[@]}"
  log_info "Complete list of paths to read: $PATHS_TO_READ"
  log_info "Complete list of paths to write: $PATHS_TO_WRITE"
}
...
handle_command "$@"
#configure_paths_to_sync docker-compose.development.yml
#load_paths_from_docker_compose docker-compose.development.yml "write"
#find_path_to_read_index /Users/mickaelperrin/tmp/drupal_theming3/src/test10
#watch